...
Customize `conf-users` and `conf-groups`, if needed, and note the names. Then build, stage, and create a tarball:
    ...
    $ make package
    $ tar -C ${DESTDIR} -czf notqmail-bin-1.08.tar.gz .

### On an install host

Copy over `notqmail-bin-1.08.tar.gz`, `instchown`, and `instcheck` from the build host. Extract the tarball -- e.g., with GNU Tar:

    # tar -C / --no-overwrite-dir -xzf notqmail-bin-1.08.tar.gz
